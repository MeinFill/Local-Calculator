        ...
        try {
            ...
            if (output.length() > 0) { // проверка на наличие ввода
                if (output.length() < 20000)
                {
                    ...
                    errors = check_tochek(output, check_tocki); // метод прверяет на несколько точек в числе. НИЧЕГО НЕ МЕНЯЕТ
                    if (check && errors.length() == 0) {
                        // если все в порядке, и у переменной число, то вывожу число
                        if (calculate(clon, check_tocki) > -2000000000000 && calculate(clon, check_tocki) < 2000000000000) {
            ...
            res.set_content("Слишком много", "enctype");
            throw;
        }
        });
