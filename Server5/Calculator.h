...
void rewrite(int&, int&, int&, string*&, int*&, long long*&);
...
void delenie_deistv_ot_chisel(string&, string&, long long*&, string*&, int*&, int&);
void schet_viragenia(int&, string&, long long*&, string*&, int*&);
auto calculate(string&);

static void rewrite(int& max_index, int& kolvo_deistv, int& i, string*& deistv, int*& deistv_weight, long long*& numbers) // перезапись большого количества данных в процессе работы калькулятор. Все перезаписывает налево на 1 символ
{
...
void delenie_deistv_ot_chisel(string& input, string& buf, long long*& numbers, string*& deistv, int*& deistv_weight, int& kolvo_numbers)
{
...
void schet_viragenia(int& kolvo_deistv, string& input, long long*& numbers, string*& deistv, int*& deistv_weight)
{
...
auto calculate(string& input) {
	...
	if (check == 1) //если ошибок нет, можно че-то делать
	{
		...
		long long* numbers = new long long[kolvo_deistv + 1]; // создание массивов чисел, действий и весов этих действий
